...
import {Accordion, Col, Figure, Nav, Row, Tab, Table, Tabs} from "react-bootstrap"
            ...
            {post && (
                <div className={'text-justify'}>
                    ...
                    <div className={"mb-4"}>
                        ...
                        <Figure className={'border-start border-end border-gray overflow-auto w-100 m-0 p-0'}>
                            <Table className={"fs-8 table-striped table-responsive m-0 w-100"}>
                                <thead className={'text-center text-wrap'}>
                                <tr className={"table-dark table-bordered"}>
                                    <td className={"vertical-center p-1 border-light border-end"}>Фаза возвышения</td>
                                    <td className={"vertical-center p-1 border-light border-end"}>Уровень</td>
                                    <td className={"vertical-center p-1 border-light border-end"}>Базовое HP</td>
                                    <td className={"vertical-center p-1 border-light border-end"}>Базовая атака</td>
                                    <td className={"vertical-center p-1 border-light border-end"}>Базовая защита</td>
                                    <td className={"vertical-center p-1 border-light border-end"}>{post.sp_stat}</td>
                                    ...
                                    <Nav variant="pills"
                                        ...
                                        {post.skills.map((skill) =>
                                            <Nav.Item key={skill.id}>
                                                <Nav.Link
                                                    className={'w-100 border-start border-end border-top rounded-0 list-item d-flex align-items-center'}
                        ...
                        <Row className='justify-content-center d-flex overflow-hidden m-1 bg-transparent w-100'>
                            <Tab.Container id="skills" defaultActiveKey="first">
                                    ...
                                    <Nav variant="pills"
                                        ...
                                        {post.constellations.map((constellation) =>
                                            <Nav.Item key={constellation.id}>
                                                <Nav.Link
                                                    className={'border-start border-end border-top rounded-0 list-item d-flex align-items-center'}
                                                    key={constellation.id} eventKey={constellation.tab_id}>
                                                    <SkillItem key={constellation.id} icon={constellation.icon}
                                ...
                                <Col xs={12} md={7} xl={8} sm={12} lg={7}
                                    ...
                                    <Tab.Content>
                                        {post.constellations.map((constellation) =>
                                            <Tab.Pane key={constellation.id} eventKey={constellation.tab_id}>
                                                <SkillDesc classname={'py-2'} list={constellation.list}

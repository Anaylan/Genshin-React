...
const Character = () => {
	...
	useEffect(() => {
		try {
			if (post != null) {
				document.title = post.body + " | " + nameApp;
			}
		} catch (error) {
			console.log(error);
		}
			...
			<>
				{post && (
					<div className={"text-justify"}>
						...
						<div className='mb-4'>
							<h2>Таланты</h2>
							<Row className='justify-content-center justify-self-center d-flex overflow-hidden m-0 bg-transparent w-100'>
											...
											{post.skills.map((skill) => (
												<Nav.Item key={skill.id}>
													<Nav.Link
														...
														eventKey={skill.id}>
														<SkillItem icon={skill.icon} title={skill.title} />
													</Nav.Link>
												</Nav.Item>
											))}
										</Nav>
